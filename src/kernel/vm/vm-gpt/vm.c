/****************************************************************************
 *      $Id: vm.c,v 1.3 1998/01/22 05:48:35 kevine Exp $
...
void vm_tcb_insert(as_t *a, dword_t v, dword_t paddr)
{
  ...
  if (v & FRAMESIZE)
  {
    l1->pte = paddr >>  FRAME_SIZE_SHIFT << 6 |
      EL_Cache_CNWb |  EL_Valid | EL_Dirty | EL_Global;
    
    l2 = gpt_insert(a, v - FRAMESIZE);
    if (!(l2->pte &  EL_Global))
    {
      l2->pte = INVALID_TCB_BASE >>  FRAME_SIZE_SHIFT << 6 |
  ...
  do
  {
	    ...
	    )
	{
	  if (vaddr >= base)
	  {
	    ...
	    if (((gptr->u.lf.pte & EL_PFN) << 6) != INVALID_TCB_BASE)
	    {
#ifdef KDEBUG
	      if (((gptr->u.lf.pte & EL_PFN) << 6) == 0)
	      {
		panic("L4 PANIC: found invalid PTE in tcb space"); /* ok */
	      ...
	      tcb_frame_free((void *)vaddr,
			     (void *) ((gptr->u.lf.pte & EL_PFN) << 6));
	      gptr->u.lf.pte = INVALID_TCB_BASE >>  FRAME_SIZE_SHIFT << 6 |
		EL_Cache_CNWb |  EL_Valid | EL_Global;
